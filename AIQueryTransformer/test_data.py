...
if __name__ == "__main__":
    ...
    # Extract first 20 entries
    df = df[:20]
    # Initialize the OpenAIDSL class
    ...
    predictions = []
    for request in df['input']:
        print("request: " + request)
        try:
            extracted_items = openai_dsl.extract_items_from_request(request)
            
            # Format extracted items into the required structure
            prediction_items = []
            ...
            prediction = ', '.join(prediction_items)
    ...
    references = [[ref.split()] for ref in df['output']]
